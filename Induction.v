...
Proof.
  ...
  (* By induction on n *)
  induction n as [| n' IHn'].
  (** First, we suppose that [n = 0], we must show that 
      0 + m = m + 0. 
  **)
  (* This follows directly from from m = m + 0 and... *)
  - rewrite -> add_0_r. 
  (* the definition of [+] *)
    reflexivity.
  (** Next, suppose [n = S n'], and assume that [ n + m = m + n ],
  we must show that 
    (S n' + m) = m + (S n')
  **)
  (* By the definition of [+], we rewrite to S (n' + m) = m + S n'*)
  - simpl. 
  (* Using m + S n' = S (m + n'), we now must show S (n' + m) = S (m + n')*)
    rewrite <- plus_n_Sm. 
...
Proof.
  ...
  {
    (* By the definition of `leb`, (S p' + n <=? S p' + m) = (p' + n <=? p' + m) *)
    simpl.
    rewrite IHp'.
    reflexivity.
  }
