...
Proof.
  ...
  - destruct b eqn:Eqb.
    ...
    + simpl. intros H. rewrite H. reflexivity. 
  Qed.
  (* TODO reiew this: Why did the first simpl. reflexivity solve the problem tho?*)
...
Proof.
  intros f.
  intros H. intros x. rewrite H. rewrite H. reflexivity. 
  Qed.
...
Proof. 
  ...
  Qed.
  (* NOTE: An alternater implementation of the lower_grade function
   doesn't get simplified by the simpl. 
   The way I write my functions / code changes how I prove things
   *)
(** [] *)
...
Theorem sn_eq_n_plus_1: forall n: nat, S(n) = n + 1.
Proof. intros n. induction n as [| n' IHn'].
  - reflexivity. 
  - simpl. rewrite IHn'. reflexivity.
Qed.

Theorem plus_n_Sm_over_m : forall n m : nat,
  S (n + m) = n + (S m).
Proof.
  intros n m. induction m as [| m' IHm'].
  - rewrite add_0_r. rewrite sn_eq_n_plus_1. reflexivity.
  - rewrite <- IHm'. rewrite -> 2 sn_eq_n_plus_1. rewrite (sn_eq_n_plus_1 m'). rewrite (sn_eq_n_plus_1 (m'+1 )). simpl. 
  Admitted.

Theorem plus_n_Sm_induction : forall n m : nat,
  S (n + m) = n + (S m).
Proof.
...
Proof.
  ...
  - simpl. rewrite IHn'. rewrite plus_n_Sm_over_m. reflexivity.
  Qed.

Theorem add_comm_m : forall n m : nat,
  n + m = m + n.
Proof.
  intros n m. induction m as [| m' IHm'].
  - simpl. rewrite add_0_r. reflexivity.
  - simpl. rewrite <- IHm'. rewrite plus_n_Sm_over_m. reflexivity.
  Qed.
...
Proof.
  intros n. induction n as [| n' IH].
  - simpl. reflexivity.
  - rewrite IH.  rewrite negation_fn_applied_twice.
    + simpl. reflexivity.
...
Proof.
  ...
  assert (H': m + n = n + m). 
  {rewrite add_comm. reflexivity. }
  rewrite -> H. rewrite add_assoc. rewrite H'. reflexivity.
  Qed.
...
Lemma mult_dist_over_1 : forall n m: nat, n * (1 + m) = n + n*m.
Proof.
  intros n m.  induction n as [|n' IH].
  - simpl. reflexivity.
  - simpl. 
  assert (H: S m = 1 + m).
  {rewrite sn_eq_n_plus_1. rewrite add_comm. reflexivity. }
  rewrite H. rewrite IH. rewrite add_shuffle3. reflexivity.
  Qed.
...
Proof.
  ...
  - simpl. rewrite sn_eq_n_plus_1. 
  assert (H: m' + 1 = 1 + m').
  {rewrite add_comm. reflexivity. }
  rewrite H. rewrite mult_dist_over_1.
...
(* simpl should be enough? prob not, induction*)
...
(* simpl should be enough due to the definition of nat? *)
Theorem zero_neqb_S : forall n:nat,
  0 =? (S n) = false.
Proof.
  intros n. simpl. reflexivity. Qed.

(* if i have andb comm, simpl / rewrite else destruct*)
Theorem andb_false_r : forall b : bool,
  andb b false = false.
Proof.
  intros b. rewrite andb_commutative. simpl. reflexivity. Qed.

(* now this is messy. idk. i think if i can have equality commutation then simpl will do,
   otherwise i need to check =? def. checking the def i think simpl would do it.
*)
...
(* i think i need a theorem with simpl? 
ok according to the def, simpl would work because it would call plus then mult over 0, which then goes into 0 +. maybe i need a theorem for 0 +, depends on the definition. we will see
*)
...
what the fwig man.
 (b&c) | (!b | !c) 
 ok, this is always true.
 so long as i have enough theorems i might be able to do with rewrite. 
 easier to destruct here tbh.
*)
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
  intros b c. simpl. destruct b.
  -  simpl. (* here i could use a theorem which shows that if either elem of or is true, then the or is true*)
  (* another theorem which shows that either c or !c is true*)
  (* or just destruct again *)
  {destruct c. 
  - simpl. reflexivity.
  - simpl. reflexivity.
  }
...
Theorem mult_dist_1: forall n m: nat,
  n * (m + 1) = n * m + n.
Proof. 
  intros n m. induction n as [| k IH].
  - simpl. reflexivity.
  - simpl. rewrite IH.
    assert (H0: S k = k + 1). { rewrite sn_eq_n_plus_1. reflexivity. }
    rewrite H0. 
    assert (H1: 1 + (k * m + k) = k * m + k + 1). { rewrite add_comm.  reflexivity. } 
Qed.

(* with all the theorems i have so far, i can prob use rewrite
no there's nothing to deal with mult like that, i need induction
*)
...
Proof.
  ...
  (* (n + m) * S p' = n * Sp' + m*Sp' *)
  (* commutate mult terms in order for the Sucessor to be the first term
  this allows our definition of sum to match and simplify
  *)
  - assert (Hn: n * S p' = S p' * n). { rewrite mul_comm. reflexivity. } 
  assert (Hm: m * S p' = S p' * m). { rewrite mul_comm. reflexivity. } 
  
  rewrite mul_comm. simpl.  rewrite mul_comm.
  rewrite IH. rewrite sn_eq_n_plus_1. rewrite mult_dist_1. rewrite mult_dist_1.
  rewrite <- add_shuffle3. 
  assert (H1: m*p' + m = m + m*p'). { rewrite add_comm. reflexivity. } rewrite H1.
...
Proof.
  intros b. induction b as [| a' IH0 | a' IH1].
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite IH1. 
  rewrite add_0_r. rewrite add_shuffle3. 
  simpl. rewrite add_0_r. reflexivity.
  Qed.
(** [] *)
...
(* 
Any innermost B0 Z are equivalent
eg. B1 Z is the same as B1 B0 Z which is equal to B1 B0 B0 Z
*)
...
Example test_normalize_3: (normalize (B1 (B1 (B0 (B0 Z))))) = (B1 (B1 (B0 Z))).
Proof. simpl. reflexivity. Qed.

Compute (bin_to_nat (B1 (B0 (B1 (B0 (B0 Z)))))).
Compute (bin_to_nat (B1 (B0 (B1 (B0 (B0 (B0 Z))))))).
...
Proof.
  intros b. induction b as [| b' IH0| b'' IH1].
   - simpl. reflexivity.
   - simpl. admit.
   - 
